...
var utilityArray = [
	...
	"function from1d2(arr, len) {  var containArr = []; for (i = 0; i < arr.length; i = i + len) {    var tArr = [];    for (d = i; d < (i + len); d++) {      if (arr[d] != undefined) {        var d2 = tArr.push(arr[d]);      }    }    containArr.push(tArr)  }  return containArr; }";
	"function downloadr(arr2D, filename) {  if (/\.csv$/.test(filename) === true) {    var data = arr2D.map(itm => {      return itm.toString().replace(/$/, '\r');    }).toString().replace(/\r,/g, '\r');  }  if (/\.json$|.js$/.test(filename) === true) {    var data = arr2D;    var type = 'data:text/plain;charset=utf-8,';  } else {    var type = 'data:application/json;charset=utf-8,';  }  var data = arr2D.map(itm => {    return itm.toString().replace(/$/, '\r');  }).toString().replace(/\r,/g, '\r');  var file = new Blob([data], {    type: type  });  if (window.navigator.msSaveOrOpenBlob) {    window.navigator.msSaveOrOpenBlob(file, filename);  } else {    var a = document.createElement('a'),      url = URL.createObjectURL(file);    a.href = url;    a.download = filename;    document.body.appendChild(a);    a.click();    setTimeout(() => {      document.body.removeChild(a);      window.URL.revokeObjectURL(url);    }, 10);  }}",
